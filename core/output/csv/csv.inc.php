 ...
 */
	...
	$id = "csv";
	$name = "Csv";
	$description = "Exportacion de datos al formato CSV";

	/**
	 * Plugin parameters
	 */
	$parameters = array();
	$parameter = array("id"=>"name","name"=>"Nombre del fichero","type"=>"text");  array_push($parameters,$parameter);
 ...
 * @return void
 */
function output_plugin_csv_execute($parameters_)
{
	// Parameters
 ...
 */
function output_plugin_csv_fetch($id_)
{
	global $conection;
	...
	$result = mysql_query($sql, $conection) or die("Error SWRA-PLUGIN-OUTPUT-CSV-001: Data query ".mysql_error());
 ...
 */
function output_plugin_csv_extract($data_)
{
	$result = array();
